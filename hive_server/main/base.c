...
#define TEST_DEVICE_NAME            "HIVE_SERVER"
...
static void watch_notify_task(void *params)
{
    gpio_pad_select_gpio(NOTIFY_GPIO);
    /* Set the GPIO as a push/pull output */
    gpio_set_direction(NOTIFY_GPIO, GPIO_MODE_INPUT);
    while(1) {
        if(gpio_get_level(NOTIFY_GPIO)) {
            //send notification
            uint8_t notify_data[15];
            for (int i = 0; i < sizeof(notify_data); ++i)
            {
            ...
            for(int i = 0; i < PROFILE_NUM; i++){
                if(gl_profile_tab[i].gatts_if && gl_profile_tab[i].conn_id && gl_profile_tab[i].char_handle){
                    esp_ble_gatts_send_indicate(gl_profile_tab[i].gatts_if,gl_profile_tab[i].conn_id, gl_profile_tab[i].char_handle,
                                        sizeof(notify_data), notify_data, false);
                }
            }
...
void app_main(void)
{
    ...
    xTaskCreate(&watch_notify_task, "watch_notify_task", 4096, NULL, 3, NULL);
